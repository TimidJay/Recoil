PlayState2 = class("PlayState2")

--TODO: Test the new variable level system and camera 

function PlayState2:initialize(level)
	playstate = self

	self.level = level

	self.className = "PlayState2"

	self.mode = "play"
	self.state = "playing" --playing, death, victory


	self.player = Player:new(level.fullWidth/2, level.fullHeight/2)
...
function PlayState2:close()
	game:clearObjects()
	love.mouse.setCursor()
	playstate = nil
end

function PlayState2:restart()
end

--should only be called on play mode
function PlayState2:nextLevel()
end

--PlayState2 has to load directly from file too
function PlayState2:loadLevel(filename)
end

function PlayState2:getOverlap()
	local overlap = {}
...
function PlayState2:setTileGrid()
...
function PlayState2:getAdjTiles(player)
...
function PlayState2:getRaycastShapes()
	local shapes = {}
